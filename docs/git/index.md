...
首先它俩都是版本控制系统，用于记录文件的历史版本。

- 架构差异：git 是分布式的版本控制系统，svn 不是。
- 存储方式：git 是基于元数据的方式来存储，svn 是基于文件差异的方式。
- 操作指令：git 指令比 svn 多，但更灵活。
- git更加灵活高效： git 相比于 svn，在处理大文件和二进制文件的版本控制上更加高效。
- 冲突解决：git 相比于 svn，更容易处理冲突。
...
- **git rebase**：‌变基操作本质‌：将当前分支的提交提取为补丁，依次应用到目标分支的最新提交后，形成线性历史，两个分支在合并到时候，会将整个分支合并到另一个分支的顶端。首先找到两个分支的共同commit记录，然后提取之后所有的commit，然后将这个commit记录添加到另一个分支的最前面，两个分支合并后的commit记录就变成线性记录 
...
‌与merge的对比‌：
merge保留原始分支结构，生成合并提交；rebase则重构历史，避免分叉。‌‌‌‌
两者最终代码状态一致，但历史记录形态不同（分叉 vs 线性）。‌‌