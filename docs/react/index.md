...
受控组件与非受控组件的核心区别在于‌状态管理方式‌。


- **受控组件**：表单元素的数据是由 React 的 State 来管理。

**优点**
1. 状态完全可控，可通过props或state强制修改输入值。
2. 便于复杂交互（如实时验证、自动完成），适合多字段表单。

**缺点**
1. 代码冗长，每个表单元素都需要编写事件处理函数。
2. 有额外性能开销，每次输入都会触发重新渲染。
...
**优点**
1. 代码简洁，无需编写事件处理函数。
2. 性能更好，无需每次输入都触发重新渲染。

**缺点**
1. 状态不完全可控，用户可以直接修改 DOM。
2. 不适合复杂交互，如实时验证、自动完成。
