...
Webpack 是一个现代的前端**模块打包工具**，它用于构建和优化 Web 应用程序的前端资源，包括 JavaScript、CSS、图片、字体等。

Webpack 的主要目标是<u>将项目的所有依赖项（模块、资源文件）打包到一个或多个最终的静态文件中</u>，以便在浏览器中加载。

核心原理：通过一系列的插件（plugins）和加载器（loaders）构建模块化 JavaScript 应用程序。

它改善了前端开发的工作流程，**提高了代码的可维护性和性能，解决了模块化、资源管理、性能优化和自动化**等多个关键问题。
